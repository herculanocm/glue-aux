...
sc = SparkContext(conf=conf2).getOrCreate()
...
df = sqlContext.read \
               .format("jdbc") \
               .option("url", f"jdbc:sqlserver://db.captalyx.com.br:26498;database=Captalys_OP;encrypt=false;trustServerCertificate=false") \
               .option("dbtable", "(select top 100 * from estoque.tb_EstoqueWexTemp) foo") \
               .option("user", 'herculano_cunha') \
               .option("password", 'Cytd2cFrYzIFjaDfQyDO') \
               .load()
